...
class MatchEnv(gym.Env):
  ...
  def __init__(
      ...
      heuristic_tiers: list[tuple[float, float]] | None = None,
  ):
    super().__init__()
    ...
    self.heuristic_tiers = heuristic_tiers
  ...
  def reset(self, seed: int | None = None, options: dict | None = None):
    ...
    if mode == "heuristic":
      if self.heuristic_tiers:
        # Uniformly picks among the defined heuristic tiers
        target_accel, target_kick = random.choice(self.heuristic_tiers)
      else:
        target_accel = self.heuristic_accel
        target_kick = self.heuristic_kick
    else:
      # Self-play models and random bots strictly use standard baseline physics
      target_accel = 3200.0
      target_kick = 1200.0
