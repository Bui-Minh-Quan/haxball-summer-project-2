...
class Dropup:
  ...
  def draw_button(
    ...
    border_color = (
        (90, 120, 175)
        if self.is_open
        else ((60, 72, 98) if is_hover else (45, 54, 75))
    )
    ...
    caret_rect = caret_surf.get_rect(
        midright=(self.rect.right - 14, self.rect.centery)
    )
  ...
  def draw_menu(self, surface: pygame.Surface, font_btn: pygame.font.Font):
    ...
    for slot in range(visible_count):
      ...
      text_color = (
          (255, 255, 255) if (is_selected or is_hover) else (180, 190, 210)
      )
      item_surf = font_btn.render(self.options[opt_idx], True, text_color)
      item_rect = item_surf.get_rect(
          midleft=(slot_rect.x + 12, slot_rect.centery)
      )
    ...
    if len(self.options) > self.max_visible:
      ...
      thumb_y = menu_rect.y + int(
          (self.scroll_offset / max_offset) * (sb_h - thumb_h)
      )
...
class MatchSetupState(GameState):
  ...
  def __init__(self, context):
    ...
    self.saved_cfg = getattr(self.context, "last_match_setup_config", None)
    self._init_dropups()
  ...
  def _init_dropups(self):
    ...
    # 1. Match Format (1v1, 2v2, 3v3)
    self.drop_format = Dropup(
        rect=pygame.Rect(left_x, 215, col_w, 42),
        label="MATCH FORMAT",
        options=["1v1 (Solo)", "2v2 (Duo)", "3v3 (Squad)"],
        values=[1, 2, 3],
        default_idx=s.get("format_idx", 0),
    )

    # 2. Player Team
    self.drop_team = Dropup(
        rect=pygame.Rect(left_x, 305, col_w, 42),
        label="PLAYER TEAM",
    ...
    self.drop_stadium = Dropup(
        rect=pygame.Rect(right_x, 215, col_w, 42),
    ...
    self.drop_time = Dropup(
        rect=pygame.Rect(right_x, 305, col_w, 42),
        label="TIME LIMIT",
        options=time_options,
        values=time_values,
        default_idx=s.get("time_idx", 3),
        max_visible=7,
    )

    # 6. Score Limit
    score_options = ["Infinite"] + [
        f"{g} Goal{'s' if g > 1 else ''}" for g in range(1, 16)
    ]
    score_values = list(range(0, 16))
    self.drop_score = Dropup(
        rect=pygame.Rect(right_x, 395, col_w, 42),
        label="SCORE LIMIT",
        options=score_options,
        values=score_values,
        default_idx=s.get("score_idx", 3),
        max_visible=7,
    )
  ...
  def _start_match(self):
    self.context.last_match_setup_config = {
        "format_idx": self.drop_format.selected_idx,
    ...
    format_size = self.drop_format.selected_value  # 1, 2, or 3
    ...
    # Assemble User Team (1 Human + (format_size - 1) AI Teammates)
    user_slots = [
        PlayerSlot(
            team=user_team,
    ...
    for i in range(1, format_size):
      teammate_ctrl, t_name = self._create_bot_controller(
          opp_type, user_team, format_size
      )
      user_slots.append(
          PlayerSlot(
              team=user_team,
              stats=PlayerStats(name=f"{t_name} T{i}"),
              controller=teammate_ctrl,
          )
      )

    # Assemble Opponent Team (format_size AI Opponents)
    opponent_slots = []
    for j in range(format_size):
      opp_ctrl, opp_name = self._create_bot_controller(
          opp_type, opp_team, format_size
      )
      opponent_slots.append(
          PlayerSlot(
              team=opp_team,
              stats=PlayerStats(name=f"{opp_name} {j+1}"),
              controller=opp_ctrl,
          )
      )

    if user_team == "red":
      red_slots = user_slots
      blue_slots = opponent_slots
    else:
      red_slots = opponent_slots
      blue_slots = user_slots

    roster = red_slots + blue_slots
  ...
  def draw(self, surface: pygame.Surface):
    ...
    t_surf = self.font_title.render("MATCH SETUP", True, (245, 245, 250))
    surface.blit(t_surf, t_surf.get_rect(center=(cx, 75)))

    sub_surf = self.font_sub.render(
        "Select team format, game rules, arena scale, and opponent AI",
        True,
        (115, 128, 155),
    )
    surface.blit(sub_surf, sub_surf.get_rect(center=(cx, 115)))

    panel_rect = pygame.Rect(cx - 390, 155, 780, 310)
...
# Compatibility alias for menu transitions
QuickPlayConfigState = MatchSetupState