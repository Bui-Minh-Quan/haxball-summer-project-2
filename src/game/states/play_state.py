...
class PlayState(GameState):
  ...
  def __init__(self, context, match_config: MatchConfig):
    ...
    self.font_player_num = pygame.font.SysFont("Arial", 20, bold=True)
    self.font_player_name = pygame.font.SysFont("Arial", 12, bold=True)
    ...
    self.kick_sounds = []
    for path in ["assets/sounds/balls/ball_kick4.wav"]:
  ...
  def _get_aa_circle(self, radius: int, fill_color, outline_color, outline_thickness=3):
    ...
    pygame.draw.circle(
        surf, outline_color, (center, center), radius * scale + outline_thickness * scale
    )
  ...
  def update(self, dt: float):
    ...
    if not self.is_match_finished:
      ...
      if goal_event is not None:
        ...
        # Score limit reached
        if mode.score_limit > 0 and (
            self.sim.score_red >= mode.score_limit
            or self.sim.score_blue >= mode.score_limit
        ):
          self.is_match_finished = True
          self.end_game_phase = "GOAL_CELEBRATION"
          self.end_game_timer = 2.2
      ...
      if self.end_game_timer <= 0.0:
        if self.end_game_phase in ("GOAL_CELEBRATION", "TIMEOUT"):
          ...
          self.exit()
          from src.game.states.match_setup_state import MatchSetupState
          self.context.state_manager.change_state(MatchSetupState(self.context))
          return

    # Camera Anchoring: Tracks Human Player across any team/format
    human_slot_idx = next(
        (i for i, s in enumerate(self.match_config.roster) if isinstance(s.controller, KeyboardController)),
        None,
    )
    if human_slot_idx is not None and human_slot_idx < len(self.sim.all_players):
      track_pos = self.sim.all_players[human_slot_idx].pos
    else:
  ...
  def _draw_player_halo(self, surface: pygame.Surface, pos: tuple[int, int], radius: int):
    ...
    if cache_key not in self._sprite_cache:
      ...
      pygame.draw.circle(hi_res, (255, 255, 255, 25), (center, center), halo_radius * scale)
      pygame.draw.circle(
          hi_res, (255, 255, 255, 120), (center, center), halo_radius * scale, width=int(3.5 * scale)
      )

      final_size = size // scale
      self._sprite_cache[cache_key] = pygame.transform.smoothscale(hi_res, (final_size, final_size))
  ...
  def draw(self, surface: pygame.Surface):
    ...
    for i, post in enumerate(p.posts):
      ...
      post_surf = self._get_aa_circle(int(post.radius), post_fill, (20, 20, 20), 3)
      surface.blit(post_surf, post_surf.get_rect(center=pos))

    # Multi-Player Rendering: Red Team
    ...
    for i, player in enumerate(self.sim.red_team):
      ...
      if slot and slot.stats.name:
        name_txt = self.font_player_name.render(slot.stats.name, True, (245, 245, 250))
        surface.blit(name_txt, name_txt.get_rect(center=(pos[0], pos[1] - int(player.radius) - 10)))

    # Multi-Player Rendering: Blue Team
    ...
    # Ball
    ball = self.sim.ball
    b_pos = cam.apply(ball.pos)
    if self.ball_img:
      surface.blit(self.ball_img, self.ball_img.get_rect(center=b_pos))
    else:
      ball_surf = self._get_aa_circle(int(ball.radius), (255, 255, 255), (20, 20, 20), 2)
      surface.blit(ball_surf, ball_surf.get_rect(center=b_pos))
  ...
  def _draw_hud(self, surface: pygame.Surface):
    ...
    pygame.draw.rect(surface, (60, 75, 100) if s_hover else (45, 55, 75), self.btn_sound_rect, border_radius=6)
    ...
    if icon:
      ...
      txt = self.font_btn.render("SND" if self.sound_enabled else "OFF", True, (240, 240, 240))
